...
void ANNbd_shrink::ann_search(ANNdist box_dist, ANNpointArray pts, ANNpoint q, double max_err, int dim, ANNmin_k * pointMK)
{
	...
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_search(inner_dist, pts, q, max_err, dim, pointMK);	// search inner child first
		child[ANN_OUT]->ann_search(box_dist, pts, q, max_err, dim, pointMK);	// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_search(box_dist, pts, q, max_err, dim, pointMK);	// search outer child first
		child[ANN_IN]->ann_search(inner_dist, pts, q, max_err, dim, pointMK);	// ...then outer child
	}
