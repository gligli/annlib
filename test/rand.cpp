...
ANNfp annRan0()
{
	...
	static ANNfp y, v[TAB_SIZE];
	static int iff = 0;
	const ANNfp RAN_DIVISOR = ANNfp(ANN_RAND_MAX + 1UL);
	if (RAN_DIVISOR < 0) {
...
{
	ANNfp wn;
	ANNfp corr_sq = correlation * correlation;

	for (int i = 0; i < n; i++) {
		ANNfp previous = annRanLaplace();
		pa[i][0] = (ANNcoord) previous;
		for (int d = 1; d < dim; d++) {
			ANNfp temp = annRan0();
			if (temp < corr_sq)
...
void annClusGaussPts(			// clustered-Gaussian distribution
	...
	ANNfp				std_dev)		// standard deviation within clusters
{
...
//
//----------------------------------------------------------------------
const ANNfp CO_FLAG = 999;						// special flag value

static void genOrthFlat(		// generate points on an orthog flat
	ANNpointArray		pa,				// point array
	int					n,				// number of points
	int					dim,			// dimension
	ANNfp				*control,		// control vector
	ANNfp				std_dev)		// standard deviation
{
	...
	if (control == NULL || new_clust) {			// need new cluster centers
		...
		for (int c = 0; c < n_clus; c++) {		// generate clusters
			...
			for (int d = 0; d < dim; d++) {		// generate side locations
				...
				ANNfp Prob = ((ANNfp) n_dim)/((ANNfp) (dim-d));
...
static void genGauss(			// generate points on a general Gaussian
	...
	int					n,				// number of points
	int					dim,			// dimension
	ANNfp				*center,		// center vector
	ANNfp				*std_dev)		// standard deviation vector
{
	for (int i = 0; i < n; i++) {
		for (int d = 0; d < dim; d++) {
...
void annClusEllipsoids(			// clustered around ellipsoids
	...
	int					n_clus,			// number of colors
	ANNbool				new_clust,		// generate new clusters.
	ANNfp				std_dev_small,	// small standard deviation
	ANNfp				std_dev_lo,		// low standard deviation for ellipses
	ANNfp				std_dev_hi,		// high standard deviation for ellipses
	int					max_dim)		// maximum dimension of the flats
{
...
void annPlanted(				// planted nearest neighbors
	...
	ANNfp			std_dev)	// standard deviation about source
{
	for (int i = 0; i < n; i++) {
