...
ANNfp			corr_coef;				// correlation coef
ANNfp			std_dev;				// standard deviation
ANNfp			std_dev_lo;				// low standard deviation
ANNfp			std_dev_hi;				// high standard deviation
int				bucket_size;			// bucket size
ANNfp			epsilon;				// error bound
int				near_neigh;				// number of near neighbors
int				max_pts_visit;			// max number of points to visit
ANNfp			radius_bound;			// maximum radius search bound
		...
		//----------------------------------------------------------------
			...
			if (stats > SILENT) {
				...
				if (stats >= EXEC_TIME) {		// output processing time
					cout << "  process_time  = "
						 << ANNfp(prep_time)/CLOCKS_PER_SEC << " sec\n";
				}
		...
		//----------------------------------------------------------------
			...
			if (stats > SILENT) {
				...
				if (stats >= EXEC_TIME) {		// print exec time summary
					cout << "  query_time    = " <<
						ANNfp(query_time)/(query_size*CLOCKS_PER_SEC)
						 << " sec/query";
					#ifdef ANN_PERF
...
void doValidation()						// perform validation
{
	...
	for (i = 0; i < query_size; i++) {			// validate each query
		...
		for (j = 0; j < near_neigh; j++) {
			...
			ANNfp true_dist = ANN_ROOT(curr_tru_dst[j]);
												// reported i-th smallest
			ANNfp rept_dist = ANN_ROOT(curr_apx_dst[j]);
												// better than optimum?
			...
			ANNfp resultErr;					// result error
			if (true_dist == 0.0) {				// let's not divide by zero
				if (rept_dist != 0.0) resultErr = ANN_FP_MAX;
				else				  resultErr = 0.0;
			}
			else {
				resultErr = (rept_dist - true_dist) / ((ANNfp) true_dist);
			}
		...
		//--------------------------------------------------------------------
			...
			for (j = 0; j < near_neigh; j++) {
				...
				while (rnk < true_nn && curr_tru_dst[rnk] <= rept_dist)
					rnk++;
				if (j+1-rnk > 0) rnkErr = (ANNfp) (j+1-rnk);
	...
	ANNbool		verbose)						// print stats
{
	...
	int too_many_levels = (int) (2.0 * st.dim * log2((ANNfp) st.n_pts));
	int opt_levels = (int) log2(ANNfp(st.n_pts)/st.bkt_size);
